...
/****************************添加标签页 开始***************************************/
    ...
    $(document).on("click", ".tabsUL li",
    function() {
        var bStopIndex = $(this).index();
        var iframe_box = $("#tabs_iframe");
        $(".tabsUL li").removeClass("tab_sed").eq(bStopIndex).addClass("tab_sed");
        iframe_box.find(".tabsIframeBlock").hide().eq(bStopIndex).show();
		//chrome scrollbar bug;
		$(".frameCon").css("height", "99%");
		$(".frameCon").width();
		$(".frameCon").css("height", "100%");
    });

    $(document).on("click", ".tabsUL li .closeIcon",
    function() {
        var $this = $(this).parents("li");
        closeTab($this);
    });

    $(document).on("dblclick", ".tabsUL li",
    function() {
        var $this = $(this);
        closeTab($this);
    });
    ...
    function closeTab($this) {
        ...
        if (aCloseIndex > 0) {
            ...
            function() {
                $this.remove();
                $('#tabs_iframe').find('.tabsIframeBlock').eq(aCloseIndex).remove();
              $(".tabsUL li").removeClass("tab_sed").eq(aCloseIndex - 1).addClass("tab_sed");
                iframe_box.find(".tabsIframeBlock").hide().eq(aCloseIndex - 1).show("fast");
                tabNavallwidth();
            });
...
function tabNavallwidth() {
    ...
    var taballwidth = 0,
    $tabNavWp = topWindow.find(".tabs_span_iner"),
    $tabNavUl = topWindow.find(".tabsUL"),
    $tabNavitem = topWindow.find(".tabsUL li"),
    $tabNavmore = topWindow.find(".tabsMore");
    ...
    $tabNavUl.width(taballwidth + 25);
    var w = $tabNavWp.width();

    if (taballwidth + 25 > w) {
        $tabNavmore.fadeIn("slow")
    } else {
        $tabNavmore.fadeOut("fast");
        $tabNavUl.css({
            left: 0
        })
    }
}tabNavallwidth();//开始测试菜单宽度

/*左菜单操作到标签页*/
function openNewtabs(obj, src, title) {
    ...
    if (src) {
        ...
        var _txt = title;
        var topWindow = $(window.parent.document);
        var show_navLi = topWindow.find(".tabsUL li");

        show_navLi.each(function() {
            if ($(this).find('a').attr("src") == _src) {
                bStop = true;
        ...
        if (!bStop) {
            creatIframe(_src, _txt);
            min_titleList();
        } else {
            show_navLi.removeClass("tab_sed").eq(bStopIndex).addClass("tab_sed");
            var iframe_box = topWindow.find("#tabs_iframe");
            iframe_box.find(".tabsIframeBlock").hide().eq(bStopIndex).show().find("iframe").attr("src", _src);
        }

    }
}

function creatIframe(_src, _txt) {
    var topWindow = $(window.parent.document);
    var show_nav = topWindow.find('.tabsUL');
    show_nav.find('li').removeClass("tab_sed");
    var iframe_box = topWindow.find('#tabs_iframe');

    show_nav.append('<li class="tab_sed"><a href="javascript:;" src="' + _src + '">' + _txt + '</a><p class="closeIcon"></p></li>');
    tabNavallwidth();
    var iframeBox = iframe_box.find('.tabsIframeBlock');
    iframeBox.hide();
    iframe_box.append('<div class="tabsIframeBlock"> <iframe class="frameCon" scrolling="yes" src="' + _src + '" frameborder="0"></iframe></div>');

    /*var showBox = iframe_box.find('.tabsIframeBlock:visible');
    showBox.find('iframe').attr("src", _src).load(function() {
...
$(document).on("click", ".reload",
    function() {
    var bStopIndex = $(".tabsUL li.tab_sed").index();
    var iframe_box = $("#tabs_iframe");
    var $Obj=iframe_box.find(".frameCon").eq(bStopIndex);
    //$Obj.attr("src",$Obj.attr("src"));
	$Obj.attr("src",$Obj[0].contentWindow.location.href);
    });
